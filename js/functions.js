...
$(function() {
	...
	$('form').on('change', "[name*='cronsRepetition'][value='timed']", function(e) {
		// If cron type is timed, hide/show appropriate fields
		var id = $(this).prop('id');
		var n = id.replace(/^[A-Za-z]+/, "");
		$(".freq"+n).hide();
		$(".timed"+n).show();
	}).on('change', "[name*='cronsRepetition'][value='freq']", function(e) {
		// If cron type is freq, hide/show appropriate fields
		var id = $(this).prop('id');
		var n = id.replace(/^[A-Za-z]+/, "");
		$(".freq"+n).show();
		$(".timed"+n).hide();
	}).on('change', "[name*='cronsFrequency'], [name*='cronsMaxRunTime'], [name*='cronsHour'], [name*='cronsMinute'], [name*='cronsWeekday'], [name*='cronsMonthday']", function(e) {
		// Validate cron params - integers for cron frequency and maxruntime inputs
		$(this).removeClass('invalid');
		$(this).next().hide();
		...
		// Prevent form submission in anything is invalid
...
var ExternalModuleTemplater = {
	elements : {
		...
		cronsForm : `<div class="cronsInfo" id="cronsInfo_i_" name="cronsInfo_i_">
				...
				<div class="form-group row freq_i_">
								<label for="cronsMaxRunTime_i_" class="col-sm-3 col-form-label">Max Run Time (s)</label>
								<div class="col-sm-6">
	...
	addTab : function(type){
		// Add a new tab to the given section 'type'
		$('#' + type + 'Card').show();
		let tabHeads = $('#' + type + 'TabHeads');
		let i = tabHeads.children().length+1;
	...
	checkButton : function(type) {
		// check the status of the remove button
	...
	removeTab : function(type) {
		...
		let cardId = '#' + type + 'Info' + (i+1);
		let infoCard = $(cardId);
		console.log('Removing ' + cardId, infoCard[0]);
		infoCard.remove();
	...
	renumberCard : function(type){
		// renumber tab heads after removing a card
